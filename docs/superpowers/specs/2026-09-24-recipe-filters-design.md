...
- **Mobile search:** its own row under the header on the list page — the same
  single input as desktop, moved below by CSS (`order-last w-full`), not a
  second input. The ingredient input lives in the filter modal only.
- **Title search** now ignores diacritics too (`normalizeText`), matching the
  ingredient search.
...
- `TIME_PRESETS`: `≤ 15` {null,15}, `15–30` {16,30}, `30–60` {31,60}, `60+` {61,null}
- `SERVINGS_PRESETS`: `1–2` {null,2}, `3–4` {3,4}, `5+` {5,null}
  (open lower bounds, so a custom "to 15" lights up the matching preset)
- `MIN_INGREDIENT_TERM = 2`
- `presetFor(range, presets)` — the preset equal to `range`, or `undefined`.
- `applyFilters(recipes, { tags, time, servings, search, ingredientIds }, taxonomy)` — tags
...
### `src/lib/utils/normalize-text.ts`

`normalizeText(s)` — lowercase, NFD, strip combining marks. Used by
`filters.ts` and the API route.
...
- Props: `label`, `presets`, `value: Range | null`, `onChange`. The unit is
  part of the label ("Total time (min)").
...
- Header wraps on mobile; the search input is visible at every width and
  sits on its own row below the title and buttons under `sm`.

### i18n

New keys in `messages/en/recipes.json` and `messages/sk/recipes.json`:
section labels (Ingredient, Total time (min), Portions), ingredient
placeholder, ingredient error, "Custom", from/to placeholders. `errors.json`:
`queryTooShort`.
...
- `useIngredientSearch.test.ts`: debounce, short term, error, abort of stale
  requests.
- `RecipeList.test.tsx`: ingredient filter via mocked fetch, range filter,
