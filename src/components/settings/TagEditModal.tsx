...
export function TagEditModal({ tag, onSave, onDelete, onClose }: TagEditModalProps) {
  ...
  async function handleSave() {
    ...
    if (!trimmed) {
      setError(t('tagEditModal.emptyNameError'))
      return
    }
    setSaving(true)
    setError(null)
    ...
    setSaving(false)
    if (ok) onClose()
    else setError(t('tagEditModal.saveError'))
  }
  ...
  async function handleDelete() {
    ...
    setDeleting(false)
    if (ok) onClose()
    else setDeleteError(t('tagEditModal.deleteError'))
  }
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow-xl p-5 w-80 flex flex-col gap-4">
        <div>
          <label className="text-xs text-gray-500 mb-1 block">{t('tagEditModal.nameLabel')}</label>
          <input
            autoFocus
            value={name}
        ...
        <div>
          <label className="text-xs text-gray-500 mb-1 block">{t('tagEditModal.colorLabel')}</label>
          <div className="flex flex-wrap gap-1.5">
            ...
            <button
              ...
              title={t('tagEditModal.noColorTitle')}
            >
        ...
        <div className="flex items-center justify-between gap-2 pt-1">
          <button
            ...
            title={t('tagEditModal.deleteTitle')}
          >
            <Trash2 size={16} />
          </button>
      ...
      {confirmingDelete && (
        <ConfirmModal
          message={
            tag.count > 0
              ? t('tagEditModal.removeFromRecipesConfirm', { name: tag.name })
              : t('tagEditModal.deleteUnusedConfirm', { name: tag.name })
          }
          confirmLabel={t('tagEditModal.deleteConfirmLabel')}
          pending={deleting}
          error={deleteError}
